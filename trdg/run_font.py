...
def main():
    ...
    # 针对每个字体创建一个单独的文件夹并生成图像
    for font in fonts:
        font_name = os.path.splitext(os.path.basename(font))[0]
        font_output_dir = os.path.join(args.output_dir, font_name)
        ...
        # 如果使用name_format=2, 生成labels.txt文件
        if args.name_format == 2:
            with open(
                os.path.join(font_output_dir, "labels.txt"), "w", encoding="utf8"
            ) as f:
